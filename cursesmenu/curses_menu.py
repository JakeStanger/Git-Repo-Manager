...
class CursesMenu(object):
    ...
    def _main_loop(self, scr):
        ...
        term_height, term_width = CursesMenu.stdscr.getmaxyx()

        # if self.items:
        #     term_height = len(self.items)

        self.screen = curses.newwin(term_height, term_width, 0, 0)
        self._set_up_colors()
        curses.curs_set(0)
    ...
    def draw(self):
        ...
        """
        self.screen.clear()  # TODO Implement this properly to prevent flickering
        self.screen.border(0)
        if self.title is not None:
            self.screen.addstr(2, 2, self.title, curses.A_BOLD)
        if self.subtitle is not None:
            self.screen.addstr(4, 2, self.subtitle, curses.A_BOLD)

        screen_rows, screen_cols = CursesMenu.stdscr.getmaxyx()

        max_items = screen_rows - 7

        display_items = self.items.copy()
        num_items = len(display_items)

        if num_items > max_items:
            if self.current_option + 7 >= screen_rows:
                start = self.current_option - max_items + 1
                display_items = display_items[start:start + max_items]
            else:
                display_items = display_items[0: max_items]

        # Display current selection
        if config.should_display_selected_option_at_top():
            self.screen.addstr(0, 0, self.items[self.current_option].show(self.current_option))

        for index, item in enumerate(display_items):
            if self.current_option == index or (self.current_option >= max_items and index == max_items-1):
            ...
            display_index = index
            if self.current_option + 7 >= screen_rows:
                start = self.current_option - max_items
                display_index = start + index + 1

            self.screen.addstr(5 + index, 4, item.show(display_index), text_style)
    ...
    def select(self):  # TODO Stop all menu options going to previous menu
        """
    ...
    def _set_up_colors(self):
        ...
        curses.init_pair(1, curses.COLOR_BLACK, config.get_highlight_color())
